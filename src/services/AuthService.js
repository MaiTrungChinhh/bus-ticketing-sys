...
const AuthService = {
  loginGoogle: async () => {
    try {
      ...
      const redirectUri = 'http://localhost:3000/api/auth/google/callback'; // URL đã cấu hình
  ...
  callbackGoogle: async ({ code, state, typeLogin = 'google' }) => {
    try {
      ...
      throw new Error(
        error.response?.data?.message || 'Không thể xử lý callback Google.'
      );
    }
  },
  loginFacebook: async () => {
        ...
        async (response) => {
          if (response.authResponse) {
            ...
            const serverResponse = await axios.post(
              `${API_BASE_URL}/auth/facebook/callback`,
              {
                accessToken,
              }
            );
