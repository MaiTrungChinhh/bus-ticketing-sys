...
import { getTotalTicketsByType } from '../../../../services/statistic'; // Giả sử bạn đã có hàm getTotalTicketsByType
...
const TicketsSold = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await getTotalTicketsByType(selectedType);
        ...
        if (response && response[0]) {
          ...
          const formattedData = data.map((item) => ({
            ...
            y: item.y, // Giá trị y (số lượng vé)
          }));
  ...
  return (
    <div className="flex justify-center p-5 max-w-3xl mx-auto bg-white shadow-lg rounded-lg">
      {/* Sidebar */}
      <div className="w-1/4 p-4 bg-gray-100 rounded-l-lg">
