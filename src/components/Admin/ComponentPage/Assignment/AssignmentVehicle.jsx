...
const AssignmentVehicle = () => {
  const [assignmentTrips, setAssignmentVehicles] = useState([]);
    ...
    const fetchData = async () => {
      try {
        ...
        if (data?.contents && Array.isArray(data.contents)) {
          ...
          setAssignmentVehicles([]); // Reset assignmentTrips nếu không đúng định dạng
          setTotalResults(0);
        }
      } catch (error) {
  ...
  const getFilteredResults = () => {
    return assignmentTrips.filter((trip) => {
