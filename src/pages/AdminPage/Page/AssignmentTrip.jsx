...
import ContentAssignmentTrip from '../../../components/Admin/ComponentPage/Assignment/ContentAssignmentTrip';
import { fetchAssignmentTrips } from '../../../services/tripService';
...
const AssignmentTrip = () => {
  const [assignmentTrips, setAssignmentTrips] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await fetchAssignmentTrips(currentPage, itemsPerPage); // Gọi API với page và pageSize
        console.log('API Response:', data); // Debug phản hồi API

        // Kiểm tra định dạng dữ liệu
        if (data?.contents && Array.isArray(data.contents)) {
          setAssignmentTrips(data.contents); // Lưu danh sách chuyến đi
          ...
          setAssignmentTrips([]); // Reset assignmentTrips nếu không đúng định dạng
          setTotalResults(0);
        }
      } catch (error) {
