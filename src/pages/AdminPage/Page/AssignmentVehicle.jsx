...
import ContentAssignmentVehicle from '../../../components/Admin/ComponentPage/Assignment/ContentAssignmentVehicle';
import { fetchAssignmentVehicles } from '../../../services/tripService';
...
const AssignmentVehicle = () => {
  const [assignmentVehicles, setAssignmentVehicles] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await fetchAssignmentVehicles(currentPage, itemsPerPage); // Gọi API với page và pageSize
        console.log('API Response:', data); // Debug phản hồi API

        // Kiểm tra định dạng dữ liệu
        if (data?.contents && Array.isArray(data.contents)) {
          setAssignmentVehicles(data.contents); // Lưu danh sách chuyến đi
          ...
          setAssignmentVehicles([]); // Reset assignmentVehicles nếu không đúng định dạng
          setTotalResults(0);
        }
      } catch (error) {
  ...
  const getFilteredResults = () => {
    return assignmentVehicles.filter((trip) => {
...
export default AssignmentVehicle;
